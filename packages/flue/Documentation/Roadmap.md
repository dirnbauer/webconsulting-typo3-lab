...
> **Delivered:** the first write surface, proven draft-only. Scoping decision (per the operator): keep DDEV unrestricted globally, **scope the sandbox to a dedicated `_flue` backend user** (admin, TSconfig `options.mcpServer.strictSandbox = 1`). New ext config `agentBackendUser` makes the control plane mint the MCP PAT as `_flue` (separate from the editor who reads the vault key), so every Flue write is forced into a draft workspace via that user's strict-sandbox. New `page-editor` agent (read + single-record `WriteTable` + `WorkspaceReview`; no publish/rollback/bulk/delete) + `page-edit` flow apply ONE requested change. **Verified safe:** editing junk page 660 left the LIVE record untouched (`title='test'`, `t3ver_wsid=0`) and staged the change in the Staging draft workspace (`t3ver_oid=660`, new title) — verdict `STAGED`. HITL = the native Workspaces module. ⚠️ **Safety depends entirely on the agent acting as a strict-sandbox user** — never set `agentBackendUser` to an unsandboxed user.
>
> **Hardening (follow-up):** a **control-plane guard is now in place** — `FlowTriggerService` refuses any write flow (declared MCP tools include a mutating tool) unless the agent backend user is strict-sandboxed (global feature flag or the user's TSconfig). A **non-admin scoped `_flue`** was attempted but deferred: as a true non-admin it can't edit the lab's existing pages without **re-permissioning the content tree** (the pages' `perms_*` are owned by other groups), which is invasive on a populated site. `_flue` stays admin but strict-sandboxed; the layered safety is draft-only writes + the guard + the dedicated identity + a minimal tool allowlist (no publish/rollback/bulk/delete). True least-privilege is a deliberate page-permission setup for a real deployment.
