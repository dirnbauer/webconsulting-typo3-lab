...
**19. A capability registry, not hand-rolled endpoints** — 🌱 `typo3-capability-manifest`, sg_apicore
The WordPress Abilities API proved the architecture: one typed, permissioned registry of what the CMS can do; MCP tools, REST routes, CLI commands and chat tools become *projections* of that registry. This is the architectural bet that outlives any single protocol. *Next: define the registry schema (name, contract, scopes, risk tier, side effects), generate the MCP tool list from it, and propose it to the TYPO3 AI initiative (→ 23).*
