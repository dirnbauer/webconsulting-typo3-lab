...
| ~~`hn/typo3-agent`~~ (installed 2026-09-16, retired 2026-09-18) | Was the upstream backend chat. Retired together with the Webconsulting chat fork `typo3-ai-chat`: the backend chat became `webconsulting/typo3-shadcn-ui` and, on 2026-09-23, its native successor `webconsulting/typo3-ai-assistant`, which runs this installation's own MCP tools in-process through nr-llm's agent runtime; see [AI Assistant](ai-assistant.md) |
...
| 2 | Identity for agents | ✅ PATs, OAuth+PKCE, capability manifests, **backend-user-bound scoped API tokens** (sg_apicore `be_user_uid`, expiring, scoped) · ⭕ agent users/consent | Agent-user concept with delegation + consent records |
...
| 11 | MCP chatbot / assistant | ✅ Upstream TYPO3 Agent: backend task module, tools in-process via typo3-mcp-server | Keep it as one command surface over governed tools; add shared trace/eval storage rather than another chat |
...
*Shipped:* `webconsulting/typo3-abilities` — `#[AsAbility]` registry schema (name, JSON-Schema contract, `resource:operation` scopes, capability-manifest risk tiers and side-effect vocabulary), one governed execution pipeline (policy gate → input validation → scopes → permission → execute → output validation), `config/abilities-policy.yaml` with deny/review_required (HITL)/max_risk_tier. CLI, MCP, and REST infrastructure remain part of the base lab. A connector-enabled validation run proved all four projections against 18 abilities (system + news/pages/content/workspaces), including the desktop editor and its human approval path. As of 2026-07-10 the private desktop connector is optional and no longer a required Composer dependency, so a clean public checkout does not require private repository credentials. Every execution attempt remains traceable through `tx_abilities_trace` on the enabled surfaces. 88 unit tests, PHPStan max.
How this completes the capability-manifest security envelope: `docs/wordpress-abilities-vs-capability-manifests.md`. *Next: promote policy YAML to TCA records (→ 15); ability-vs-manifest cross-check audit rule; the 2026-07-28 MCP-spec projection; propose to the TYPO3 AI initiative (→ 23).*
