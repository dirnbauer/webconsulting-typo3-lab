...
*Shipped:* `webconsulting/typo3-abilities` — `#[AsAbility]` registry schema (name, JSON-Schema contract, `resource:operation` scopes, capability-manifest risk tiers and side-effect vocabulary), one governed execution pipeline (policy gate → input validation → scopes → permission → execute → output validation), `config/abilities-policy.yaml` with deny/review_required (HITL)/max_risk_tier. **All four projections verified live in the lab against 18 abilities** (system + news/pages/content/workspaces from the desktop connector): CLI (`abilities:list|describe|run`), MCP (compiler-pass `ability_*` tools in `mcp:tool:list`), REST (`GET/POST /api/abilities/v1/abilities…` on backend-user-bound sg_apicore tokens), and the **desktop editor** (the TYPO3 Desktop Editor's news/pages/workspace operations delegate to the registry; wire format unchanged). The HITL gate is proven: `workspaces/publish` (High risk) is denied by the `review_required` policy and runs only with an explicit approval (`--approve-review` on CLI, the app's publish confirmation over the desktop surface). Every attempt is traced (`tx_abilities_trace`, surfaces `cli`/`mcp`/`rest`/`desktop`, → 13). 88 unit tests, PHPStan max.
