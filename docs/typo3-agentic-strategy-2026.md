...
| ~~`hn/typo3-agent`~~ (installed 2026-09-16, retired 2026-09-18) | Was the upstream backend chat. Retired together with the Webconsulting chat fork `typo3-ai-chat`: the backend chat is now `webconsulting/typo3-shadcn-ui`, which runs this installation's own MCP tools in-process through nr-llm's agent runtime; see [shadcn UI and backend chat](shadcn-ui.md) |
