...
Coolify does not deploy when this repository is pushed: the repository has no
webhook and no deploy key, so the running containers stay on whichever commit
was last deployed by hand, however green CI is. `status` prints that commit as
the image tag.
