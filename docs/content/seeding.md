...
3. `ddev exec vendor/bin/typo3 desiderio:starter:seed --preset=corporate --root-map=corporate:740` (without `--hide-unmanaged-children`, which would hide the element library folder)
4. `ddev exec vendor/bin/typo3 desiderio:library:seed --parent=505 --hosts=desiderio,innesto,core`, then `--parent=740`, `--parent=1290 --hosts=astryx_typo3,core`, and `--parent=933 --locale=de`
5. `ddev exec vendor/bin/typo3 desiderio:blog:seed-pages --root=69` and `--root=390`. Never without `--root`: it would seed English demo posts into every blog.
