...
mkdir -p /run/typo3-secrets /var/www/html/public/fileadmin /var/www/html/var

if [ ! -s /run/typo3-secrets/encryption-key ]; then
    php -r 'echo bin2hex(random_bytes(48));' > /run/typo3-secrets/encryption-key
fi

chown -R www-data:www-data /run/typo3-secrets /var/www/html/public/fileadmin /var/www/html/var
